...
| Stream page live (demo) | Lawrence: "publish to https://stream.myindsound.com/stream" (2026-09-16). Published as manual Netlify deploys (latest `6aab90a7b6fe47b214851abd`, 2026-09-17, back hub opening + spindle): the live site's files and functions from commit `1e5c3df`, byte-for-byte, with only `/stream` swapped for the 3D player in demo mode (`VITE_PLAYER_DEMO=1`, demo clip, no sign-in). Rollback: restore deploy `698eeb90a077b40008274af6`. The next push to `master` replaces it. Local `master` (e61ad0f) has diverged from `origin/master` (1e5c3df); reconcile before merging the branch |
