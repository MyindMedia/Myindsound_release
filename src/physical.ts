...
class PhysicalStore {
  ...
  private renderProducts() {
    const apparelContainer = document.getElementById('products-apparel');
    const accessoriesContainer = document.getElementById('products-accessories');
    const collectiblesContainer = document.getElementById('products-collectibles');

    const apparel = PRODUCTS.filter(p => p.category === 'apparel');
    const accessories = PRODUCTS.filter(p => p.category === 'accessories');
    const collectibles = PRODUCTS.filter(p => p.category === 'collectibles');

    if (apparelContainer) {
      apparelContainer.innerHTML = apparel.map(p => this.renderProductCard(p)).join('');
      this.attachCardListeners(apparelContainer);
    }

    if (accessoriesContainer) {
      accessoriesContainer.innerHTML = accessories.map(p => this.renderProductCard(p)).join('');
      this.attachCardListeners(accessoriesContainer);
    }

    if (collectiblesContainer) {
      collectiblesContainer.innerHTML = collectibles.map(p => this.renderProductCard(p)).join('');
      this.attachCardListeners(collectiblesContainer);
    }
  }
  ...
  private renderCartItems() {
    ...
    if (items.length === 0) {
      ...
      if (subtotalEl) subtotalEl.textContent = cart.getFormattedSubtotal();
      ...
      itemsContainer.querySelectorAll('.cart-item').forEach(itemEl => {
        ...
        itemEl.querySelector('.qty-decrease')?.addEventListener('click', () => {
          ...
          if (item) cart.updateQuantity(id!, variant, item.quantity - 1);
        });
